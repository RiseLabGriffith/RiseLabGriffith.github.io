...
intro: Papers, grants, people and events from the RISE community. Seminars and reading groups are open to Griffith staff and students; external visitors are welcome by arrangement.
wide: true
---
...
<h3 class="timeline__year" id="news-{{ group.name }}">{{ group.name }}</h3>
...
<h2 id="events-heading" class="events__title">Upcoming events <span class="people-group__count">{{ upcoming_count }}</span></h2>
{%- if upcoming_count > 0 -%}
...
<summary>Past events <span class="people-group__count">{{ past_count }}</span></summary>
<div class="events__list">
